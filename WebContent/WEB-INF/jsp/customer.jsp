...
<%@ taglib prefix="d" uri="http://com.dustdawn/common/"%>
<%
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
							...
							<tbody>
								<c:forEach items="${page.rows}" var="row">
									<tr>
										...
										<c:if test="${row.cust_source=='7' }">网络营销</c:if>
										</td>
										<td>
										<c:if test="${row.cust_industry=='1'}">教育培训</c:if>
										<c:if test="${row.cust_industry=='2'}">电子商务</c:if>
										<c:if test="${row.cust_industry=='3'}">对外贸易</c:if>
										<c:if test="${row.cust_industry=='4'}">酒店旅游</c:if>
										<c:if test="${row.cust_industry=='5'}">房地产</c:if>
										</td>
										<td>
										<c:if test="${row.cust_level =='22'}">普通客户</c:if>
										<c:if test="${row.cust_level=='23' }">VIP客户</c:if>
										</td>
						...
						<div class="col-md-12 text-right">
							<d:page url="${pageContext.request.contextPath }/customer/list.action"/>
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
